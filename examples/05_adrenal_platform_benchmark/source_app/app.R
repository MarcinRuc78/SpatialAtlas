...
object <- readRDS("../source_data/adrenal_visium_seurat.rds")
feature <- "Npy"
slot <- 'data'
alpha <- c(0.1, 1.0)
...
features <- sort(rownames(GetAssayData(object, slot = slot, assay = assay)))
coords <- GetTissueCoordinates(object@images$slice1)
cells.use <- Cells(object)

# Fetch initial feature data for plot
feature.data <- FetchData(object, vars = feature, cells = cells.use, slot = slot)
...
server <- function(input, output, session) {
  ...
  observeEvent(input$feature, {
    ...
    feature.data <- FetchData(object, vars = feature.use, cells = cells.use, slot = slot)
